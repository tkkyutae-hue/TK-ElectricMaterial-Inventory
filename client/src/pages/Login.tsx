...
export default function Login() {
  ...
  // Handle error codes from Google OAuth callback redirect
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const err = params.get("error");
    if (err === "pending")               setError(t.googlePending);
    else if (err === "rejected")         setError(t.googleRejected);
    else if (err === "google")           setError(t.googleError);
    else if (err === "google_not_configured") setError("Google login is not yet configured.");
  }, []);
        ...
        <div className="vs-card" style={{
          ...
          <div style={{ padding: 28 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "20px 0 16px" }}>
              <div style={{ flex: 1, height: 1, background: F.borderStrong }} />
              <span style={{ fontSize: 11, color: F.textDim, textTransform: "uppercase", letterSpacing: 1.5, fontFamily: "'Barlow Condensed', sans-serif" }}>
                {t.orDivider}
              </span>
              <div style={{ flex: 1, height: 1, background: F.borderStrong }} />
            </div>
            ...
            <a
              ...
              style={{
                ...
                background: fieldTheme === "light" ? "#fff" : "#1e293b",
                border: `1px solid ${F.borderStrong}`,
                borderRadius: 10,
                cursor: "pointer",
                fontSize: 14, fontWeight: 600,
                color: fieldTheme === "light" ? "#1e293b" : F.text,
                fontFamily: "'Barlow', sans-serif",
                textDecoration: "none",
                transition: "border-color 0.15s, box-shadow 0.15s",
              }}
              onMouseEnter={e => {
                (e.currentTarget as HTMLAnchorElement).style.borderColor = "#4285F4";
                (e.currentTarget as HTMLAnchorElement).style.boxShadow = "0 2px 12px rgba(66,133,244,0.18)";
              }}
              onMouseLeave={e => {
                (e.currentTarget as HTMLAnchorElement).style.borderColor = F.borderStrong;
