...
import { ShoppingCart, RefreshCw, CheckCircle, XCircle, AlertTriangle, AlertCircle, Package, Layers } from "lucide-react";
...
export default function Reorder() {
  ...
  // Group filtered items by category. Items whose category name cannot be
  // resolved (null categoryId, deleted category, or categories still loading)
  // all merge into the same "__none__" / "Uncategorized" bucket.
                ...
                <TableRow key={`cat-${group.key}`} className="hover:bg-transparent border-t border-slate-200 first:border-t-0">
                  <TableCell colSpan={8} className="py-1.5 px-4 bg-slate-50">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="text-[10px] font-semibold text-slate-500 border-slate-200 px-1.5 py-0 h-4">
                        {group.items.length}
                      </Badge>
                    </div>
                  </TableCell>
