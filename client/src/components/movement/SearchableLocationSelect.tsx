    ...
    <div ref={ref} className="relative" data-testid={testId}>
      ...
      >
        {open && !isMobile ? (
          ...
          <>
            <span style={D ? { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 } : undefined} className={D ? undefined : "truncate text-slate-900 flex-1"}>{displayLabel}</span>
      ...
      {open && createPortal(
        isMobile ? (
          ...
          >
            {resultList({ maxHeight: 300, overflowY: "auto" })}
          </div>
        ),
        document.body
      )}
